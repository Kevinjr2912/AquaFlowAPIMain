...
export class FilterId {
  ...
  private ensureValueIsDefined(value: string): void {
    if (value === undefined || value === null || value === "") {
      throw new InvalidArgumentError("The filter id value must be defined");
    }
  }
  ...
  private ensureValueIsUUID(value: string): void {
    ...
    if (!uuidRegex.test(value)) {
      throw new InvalidArgumentError("The filter id value does not comply with the UUID format");
    }
  }
